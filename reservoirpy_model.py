...
# Preliminary sample_reservoir_predictions on axon data
...
# squid_predict(1)
test_reservoir_py(1, reservoir_size=1571, lr=0.002001591748531257, sr=0.08189176846577592, reg=1e-07, train_time=5000, warmup=150, predict_n=5000, num_points=100000, plot_data=True, save_data=True)
...
#
# sample_reservoir_predictions = np.empty((train_time, 1))
# init = test_warmup[-1].reshape(1, -1)
#
# for i in range(len(train_data)):
#     init = esn(init)
#     sample_reservoir_predictions[i] = init
#
# print(
#     "Root mean-squared error:", rmse(data[train_time:2 * train_time + 1], sample_reservoir_predictions),
#     "R^2 score:", rsquare(data[train_time:2 * train_time + 1], sample_reservoir_predictions)
# )
#
#
# np.savetxt(f"sample_reservoir_predictions/axon_{axon_num}_test_data.csv", data[train_time + 1:2 * train_time], delimiter=",")
# np.savetxt(f"sample_reservoir_predictions/axon_{axon_num}_prediction.csv", sample_reservoir_predictions, delimiter=",")
#
# plt.figure(figsize=(10, 3))
# plt.title("SQUID Training Data: ReservoirPy")
# plt.xlabel("$t$")
# plt.plot(data[train_time + 1:2 * train_time], label="Test Data", color="blue")
# plt.plot(sample_reservoir_predictions, label="Prediction", color="red")
# plt.legend()
# plt.show()
# plt.savefig(f"sample_reservoir_predictions/axon_{axon_num}_timeseries.png")
...
# f = open("sample_reservoir_tuning_trials/size_grid_search.json","w")
# f.write(json)
# f.close()
...
# f = open("sample_reservoir_tuning_trials/ridge_grid_search.json","w")
# f.write(json)
# f.close()
...
# f = open("sample_reservoir_tuning_trials/spectral_rad_grid_search.json","w")
# f.write(json)
# f.close()

# test_data = np.loadtxt(f"sample_reservoir_predictions/axon_1_test_data.csv", delimiter=",")
# sample_reservoir_predictions = np.loadtxt(f"sample_reservoir_predictions/axon_1_prediction.csv", delimiter=",")
...
# sample_reservoir_predictions = esn.fit(X[:500], X[1:501]).run(X[501:-1])
# print("RMSE:", rmse(X[502:], sample_reservoir_predictions), "R^2 score:", rsquare(X[502:], sample_reservoir_predictions))
